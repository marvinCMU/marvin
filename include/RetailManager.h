...
#ifndef RETAILMANAGER_H_
#define RETAILMANAGER_H_
...
public:
	...
	RetailObject (std::string i, std::string n, std::string c, std::string p, std::string r, std::string o) {
		id = i;
		name = n;
		color = c;
		price = p;
...
class RetailManager {
public:
	std::vector<RetailObject> objVec;
	std::vector<std::string> cmdVec;
	std::string libPath;

	RetailManager (std::string path) {
		libPath = path;
	}
	...
	std::string loadObject () {
		std::string objPath = libPath + RETAIL_OBJ_XML;
		std::string objStr = "";
		objVec.clear();
		...
		if (!objFile.is_open()) {
    		std::cout << RETAIL_MAN_HEAD << "File " << objPath << " cannot be opened!" << std::endl;
			return "";
		}
		objFile.close();
		...
		BOOST_FOREACH( boost::property_tree::ptree::value_type const& v, tree.get_child("root") ) {
		    if ( v.first == "retail" ) {		
				...
				std::string price = v.second.get<std::string>("price");
				std::string review = v.second.get<std::string>("review");
				std::string option = v.second.get<std::string>("option");
                changeWordCase(id, -1);
		        objVec.push_back (* new RetailObject(id, name, color, price, review, option));
		    }
		}

		// return object string
		for (unsigned int i = 0; i < objVec.size(); i++) {
			objStr = objStr + objVec[i].id;
			if (i != objVec.size()-1)
				objStr = objStr + " ";
		}
		return objStr;
	...
	// return the object if the the object id found
	RetailObject* findObject (std::string id) {
		for (unsigned int i = 0; i < objVec.size(); i++) {
	...
	// insert a new object
	bool insertObject (std::string id, std::string name, std::string color, std::string price, std::string review, std::string option) {
		objVec.push_back (* new RetailObject(id, name, color, price, review, option));
		updateObjectFile ();
		return true;
	}
	...
	void updateObjectFile () {
		...
		// xml data
		for (unsigned int i = 0; i < objVec.size(); i++) {
			// convert url meta chars
			std::string sr = replaceChar (objVec[i].review, '&', "&amp;");
			std::string so = replaceChar (objVec[i].option, '&', "&amp;");
			objFile << "\t<retail id=\"" << objVec[i].id << "\">" << "\n";
			objFile << "\t\t<name>" << objVec[i].name << "</name>" << "\n";
			objFile << "\t\t<color>" << objVec[i].color << "</color>" << "\n";
			objFile << "\t\t<price>" << objVec[i].price << "</price>" << "\n";
			objFile << "\t\t<review>" << sr << "</review>" << "\n";
			objFile << "\t\t<option>" << so << "</option>" << "\n";
			objFile << "\t</retail>" << "\n";
		}
		objFile << "</root>" << "\n";
		objFile.close();
	...
	void printObject () {
		std::cout << RETAIL_MAN_HEAD << "Objects:" << std::endl;
		for (unsigned int i = 0; i < objVec.size(); i++)
			objVec[i].print();
		std::cout << std::endl;
	}
	...
	std::string loadCommand () {
		std::string cmdPath = libPath + RETAIL_CMD_XML;
		std::string cmdStr = "";
		cmdVec.clear();
        ...
        std::string line;
		if (!cmdFile.is_open()) {
    		std::cout << RETAIL_MAN_HEAD << "File " << cmdPath << " cannot be opened!" << std::endl;
			return "";
		}
		...
		if (cmdFile.bad()) {
			std::cout << RETAIL_MAN_HEAD << "Errors while reading file " << cmdPath << std::endl;
			cmdVec.clear();
			return "";
		}
		cmdFile.close();
	...
	void printCommand () {
		std::cout << RETAIL_MAN_HEAD << "Commands:" << std::endl;
		for (unsigned int i = 0; i < cmdVec.size(); i++)
	...
	void updateCorpusFile () {
		std::string lanPath = libPath + RETAIL_LAN;
		std::string corPath = libPath + RETAIL_COR;
		std::ifstream lanFile (lanPath.c_str());
		std::ofstream corFile (corPath.c_str());
	    std::string line;
		// check files
		if (!lanFile.is_open()) {
    		std::cout << RETAIL_MAN_HEAD << "File " << lanPath << " cannot be opened!" << std::endl;
			return;
		}
		if (!corFile.is_open()) {
    		std::cout << RETAIL_MAN_HEAD << "File " << corPath << " cannot be opened!" << std::endl;
			return;
		}
		...
		if (lanFile.bad()) {
			std::cout << RETAIL_MAN_HEAD << "Errors while reading file " << lanPath << std::endl;
			return;
		}
		lanFile.close();
		corFile.close();
...
/*int main (int argc, char *argv[]) {
	RetailManager *manager = new RetailManager("/home/qifan/Dropbox/mmfps/mmfpspkg/library/lib_retail/");
	std::cout << manager->loadObject() << std::endl;
	manager->printObject();
	std::cout << manager->loadCommand() << std::endl;
	manager->printCommand();
	RetailObject* f = manager->findObject("Airborne");
	f->print();
	manager->insertObject("idx", "hi", "few", "das", "eqw", "v");
	manager->printObject();
	manager->deleteObject("idx");
	manager->printObject();
	return 0;
}*/
