...
#define INPUT_FOLDER "home/qifan/Dropbox/mmfps/mmfpspkg/node_location/reconstruction/"
#define OUTPUT_FOLDER "home/qifan/Dropbox/mmfps/mmfpspkg/node_location/output/"

void Undistortion(CvMat *K, CvMat *invK, double omega, vector<double> &vx,  vector<double> &vy)
{
	for (unsigned int iPoint = 0; iPoint < vx.size(); iPoint++)
	{
...
int main ( int argc, char * * argv )
{
	...
	for (unsigned int iDesc = 0; iDesc < vID.size(); iDesc++)
	{
	...
	for (unsigned int iDesc = 0; iDesc < vvDesc.size(); iDesc++)
	{
	...
	for (unsigned int iFile = 0; iFile < vFilename.size(); iFile++)
	{
		vector<SIFT_Descriptor> vSift_desc;
		LoadSIFTData(path+vFilename[iFile], vSift_desc);

		vector<double> vx1, vy1;
		for (unsigned int isift = 0; isift < vSift_desc.size(); isift++)
		{
		...
		for (unsigned int iDesc = 0; iDesc < vSift_desc.size(); iDesc++)
		{
			for (int iDim = 0; iDim < 128; iDim++)
			{
		...
		for (unsigned int iPoint = 0; iPoint < vCorr.size(); iPoint++)
		{
		...
		for (unsigned int iInlier = 0; iInlier < vInlier.size(); iInlier++)
		{
		...
		for (unsigned int iPoint = 0; iPoint < vCorr_ransac.size(); iPoint++)
		{
