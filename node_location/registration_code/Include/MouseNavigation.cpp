...
static void MouseNavigation_Reshape(int w,int h);
static void MouseNavigation_Mouse(int button, int state, int x, int y);
static void MouseNavigation_Motion(int x, int y);

static void MouseNavigation_Pick(GLdouble x, GLdouble y,GLdouble delX, GLdouble delY);

/* Configurable center point for zooming and rotation */

GLfloat ReferencePoint[4] = { 0,0,0,0 };

void MouseNavigation_MouseWheel(int button, int dir, int x, int y)
{
	if (dir > 0)
	{
		_mouseDir = 1;
	...
	double s = exp((double)_mouseDir*0.1);
...
void
MouseNavigation_Init()
{
	getMatrix();

	glutReshapeFunc(MouseNavigation_Reshape);
	glutMouseFunc(MouseNavigation_Mouse);
	glutMouseWheelFunc(MouseNavigation_MouseWheel);
	glutMotionFunc(MouseNavigation_Motion);
}

static void
MouseNavigation_Reshape(int w,int h)
{
	glViewport(0,0,w,h);
...
static void
MouseNavigation_Mouse(int button, int state, int x, int y)
{
	GLint viewport[4];

	/* Do picking */
	if (state==GLUT_DOWN)
		MouseNavigation_Pick(x,glutGet(GLUT_WINDOW_HEIGHT)-1-y,3,3);
...
static void
MouseNavigation_Motion(int x, int y)
{
	bool changed = false;
	...
	//if (_mouseDir)
	//{
	//	double s = exp((double)_mouseDir*0.01);
...
static void (*selection)(vector<StaticStructure> vStaticStructure) = NULL;
static void (*pick)(GLint name) = NULL;
vector<StaticStructure> vStaticStructure;

void MouseNavigation_SelectionFunc(void (*f)(vector<StaticStructure> vStaticStructure), vector<StaticStructure> pvStaticStructure)
{
	selection = f;
	vStaticStructure = pvStaticStructure;
}

void MouseNavigation_PickFunc(void (*f)(GLint name))
{
	pick = f;
}
...
static void
MouseNavigation_Pick(GLdouble x, GLdouble y,GLdouble delX, GLdouble delY)
{
	GLuint buffer[1024];
	...
	if (selection)
		selection(vStaticStructure);                                 /* Draw the scene in selection mode */
