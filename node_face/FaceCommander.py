#!/usr/bin/env python
"""
faceCommander.py is a wrapper for pocketsphinx
used to detect faceCommander defined
  parameters:
...
class FaceCommander(object):
    """ GStreamer based speech recognizer. """

    def __init__(self):

        """ Initialize the speech pipeline components. """
        rospy.init_node('faceCommander')
        ...
        asr.set_property('fwdflat', True)
        asr.set_property('bestpath', True)
        asr.set_property('dsratio', 1)
        asr.set_property('maxhmmpf', 3000)
        asr.set_property('maxwpf', 20)

        # parameters for lm and dic
        try:
            lm_ = rospy.get_param('~lm')
        except:
            rospy.logerr('<Speech> faceCommander: Please specify a language model file')
            return
        try:
            dict_ = rospy.get_param('~dict')
        except:
            rospy.logerr('<Speech> faceCommander: Please specify a dictionary')
            return
    ...
    def shutdown(self):
        ...
        rospy.loginfo("<Speech> faceCommander node closed")
        gtk.main_quit()

    def start(self, msg):
        self.pipeline.set_state(gst.STATE_PLAYING)
        print '<Speech> faceCommander node resumed'
        return EmptyResponse()

    def stop(self, msg):
        self.pipeline.set_state(gst.STATE_PAUSED)
        print '<Speech> faceCommander node paused'
        return EmptyResponse()
