function [m,other] = Gsvm_update_svm(m,D,alpha,lambda2)
...
M = size(newx,1);
sig = D*alpha;
P = diag(1./sqrt(2*lambda2./sig+ones(M,1)))*newx;


svm_model = libsvmtrain(supery, P',sprintf(['-s 0 -t 0 -c' ...
...
if length(svm_model.sv_coef) == 0
  ...
  wex = svm_weights';
  wex = diag(1./sqrt(2*lambda2./sig+ones(M,1)))*wex;
  b = svm_model.rho;
