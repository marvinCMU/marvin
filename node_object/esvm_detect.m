...
rs1.details.total_r_flip = rs2.details.total_r;
rs1.details.highest_response_info_flip = rs2.details.highest_response_info;
rs1.r = [rs1.r;rs2.r];
resstruct = rs1;

%Concatenate normal and LR pyramids
% feat_pyramid = cat(1,t1,t2);
feat_pyramid.orig = t1;
feat_pyramid.flip = t2;
...
[resstruct,t] = esvm_detectdriverBLOCK(I, models, params);
...
%   grad_term = zeros(size(exemplar_matrix,2),size(X,2));
%   if params.usingViewGradients 
%       % calculate the view gradient term
%       for i = 1:size(exemplar_matrix,2)
%         grad_term_sub = X'*models{i}.model.DIC_ext;
%         grad_term(i,:) = sum(grad_term_sub.^2,2)';
%       end
%       
%       % make the mean to be 0 which is equivalent of modifying the
%       % threshold
%       grad_term = grad_term - mean(grad_term(:));
%   end

    
    if params.usingViewGradients == 1
        r = zeros(length(models),size(X,2));
        for i = 1:length(models)
            r(i,:) = models{i}.model.sv_coef'*my_kernel(models{i}.model.SVs,X,params.grad_lambda);
        end
    elseif params.usingViewGradients == 2
        grad_term = zeros(size(exemplar_matrix,2),size(X,2));
        for i = 1:size(exemplar_matrix,2)
            grad_term_sub = sum((models{i}.model.DIC_coef'*X).^2,1);
            grad_term(i,:) = grad_term_sub;
        end
        r = exemplar_matrix'*X + 1/(4*params.grad_lambda)*grad_term;
    else
        r = exemplar_matrix' * X;
    end
    r = bsxfun(@minus, r, bs);
...
resstruct.details.total_r = r;
resstruct.details.hog_levels = offsets(2,:);
resstruct.details.hog_scales = t.scales;
resstruct.details.hog_padder = t.padder;
resstruct.details.uus = uus;
resstruct.details.vvs = vvs;
resstruct.details.hog_size = cellfun(@(x)size(x),t.hog,'UniformOutput',false);
resstruct.details.hog_size = cat(1,resstruct.details.hog_size{:});
...
for exid = 1:N
  ...
  if isempty(goods)
    % collect info about highest response
    [ignore bb_max]= max(r(exid,:));
    hi_level = offsets(2,bb_max);
    hi_uus = uus(bb_max);
    hi_vvs = vvs(bb_max);
    resstruct.details.highest_response_info(exid,:) = [hi_uus hi_vvs hi_level params.detect_add_flip];
    continue    
  end
  ...
  % record info about highest response
  resstruct.details.highest_response_info(exid,:) = [curuus(1) curvvs(1) levels(1) params.detect_add_flip];
