...
cv::Mat loadImage(const char* path, bool resize){
  ...
  if( !file.good()){
    ...
    std::cout<<"Warning! Couldn't load image"<<std::endl;
...
void processClick( const SDL_MouseButtonEvent* button ){
  ...
  if(gState == RESULT_OPTIONS){
    ...
    if(x > (463*factor) && x < (651*factor) && y > (968*factor) && y < (1031*factor)){
      ...
      std_msgs::String msg;
      msg.data = gFinalInfo.type + string(":") + gFinalInfo.command + (":") + gFinalInfo.object;
      publishers.feedback.publish(msg);
      //      ROS_INFO("%s:%s:%s:%s",REPORT_OUT,REPORT_GUI,REPORT_GUI_FEEDBACK,msg.data.c_str());
      setState( RESETTING );
    }
...
void callback_bnet_final_result( const std_msgs::String::ConstPtr& msg ){
  //  ROS_INFO("%s:%s:%s",REPORT_IN,REPORT_GUI,REPORT_FINAL_RESULT);
...
void initTextures(){
  ...
  string filename;
  cv::Mat UAImg;
  //filename = string(IMAGE_DIR) + string("alum_border.jpg");
  //UAImg = loadImage(filename.c_str());
  //updateTexture(gTexIds[STREAM_TEX], 0, 0, UAImg.cols, UAImg.rows, UAImg.data );
  //cv::imshow("test", UAImg);
  //  cv::waitKey(0);
