...
#define IMG_SIZE_RATIO_MULTIPLIER 2.0
...
void dummy_callback_bnet_final_result( std::string msg ){
  ...
  setResultsTexture( *gActiveResultsConfig, images, boundingBoxes );
...
void callback_bnet_final_result( const std_msgs::String::ConstPtr& msg ){
  ...
  temp = strtok( line, " "); lines.pop_front(); //commands
  while( temp!= NULL) {
    commands.push_back( string(temp ) );
    cout<<"cmd: "<<commands.back()<<std::endl;
    temp = strtok(NULL, " ");
  }
  ...
  temp = strtok( line, " "); lines.pop_front();
  while( temp!=NULL){
    objects.push_back( string(temp) ); 
    cout<<"obj: "<<objects.back() <<endl;
    temp = strtok(NULL, " ");
  }
  ...
  //cout<<"object size "<<objects.size()<<endl;
    /*switch( objects.size() ){
  case 1: config = gResultsConfig[ONE_RESULT]; break;
  case 2: config = gResultsConfig[TWO_RESULTS]; break;
  case 3: config = gResultsConfig[THREE_RESULTS]; break;
  default: config = gResultsConfig[FOUR_RESULTS]; break;
  }*/
  //setResultsTexture( config, images, boundingBoxes );
  //setState(RESULT_OPTIONS);*/
}
/**/
...
void initResultsConfigurations(){
  ...
  gResultsConfig[ONE_RESULT].push_back( Quad(0.25 ,0.25, 0.0, width, height, TOP_RIGHT) );
...
void initTextures(){
  ...
  filename = string(IMAGE_DIR) + string("initial_feedback2.jpg");
  UAImg = loadImage(filename.c_str());
  ...
  gQuads[INITIAL_APPROVAL].w = 100/(float)gScreenWidth;
  gQuads[INITIAL_APPROVAL].h = 80/(float)gScreenHeight;
